...
export function useHydratePositions({
  tokenIds,
  wallet,
  enabled,
}: {
  ...
  const positions = useMemo(() => {
    ...
    if (!data || !wallet) return [] as HydratedPosition[];
    const cards: HydratedPosition[] = [];
    for (let i = 0; i < tokenIds.length; i += 1) {
      ...
      if (classified.kind === "card") cards.push(classified.position);
    }
    return cards;
